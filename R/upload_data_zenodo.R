...
#' @param deposit_id The deposit ID of the upload. This should be the last part of the URL for newly 
#' created deposit e.g. https://zenodo.org/uploads/XXXX where XXXX is the deposit_id.
#' @return Returns exit status of curl
#' @export
...
#' Upload a file to Zenodo
#' 
#' @param files A vector with paths to files to upload.
#' @param zenodo_access_token Path to Zenodo access token. Tokens can be created at 
#' https://zenodo.org/account/settings/applications/tokens/new/ with the "deposit:write" and "deposit:actions" scopes.
#' @param deposit_id The deposit ID of the upload. This should be the last part of the URL for newly 
#' created deposit e.g. https://zenodo.org/uploads/XXXX where XXXX is the deposit_id.
#' @return Returns exit status of curl
#' @export
zenodo_upload = function(files, zenodo_access_token, deposit_id){
  ...
  # Check that path to file and access token exists
  if(any(!file.exists(files))){stop("One or more files does not exist")}
  if(!file.exists(zenodo_access_token)){stop("zenodo_access_token does not exist")}
